...
const getUserPlaylists = wrapAsyncHandler(async (req, res) => {
    ...
    const playlist = await User.aggregate([
        {
            $match: {
                _id: new mongoose.Types.ObjectId(userId), // Convert videoId to ObjectId
            },
        },
        {
            $lookup: {
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, playlist[0], "User Video playlist fetched")
        );
})
...
const getPlaylistById = wrapAsyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, playlist[0], "Video playlist fetched")
        );
...
const addVideoToPlaylist = wrapAsyncHandler(async (req, res) => {
    ...
    if (!playlistId || !videoId) {
        throw new ApiError("To add a video in playlist  you should provide both playlist and video ids")
    }
    ...
    return res.status(204)
        .json(200, playlist, "Video added to the playlist")
})
...
const removeVideoFromPlaylist = wrapAsyncHandler(async (req, res) => {
    ...
    if (!playlistId || !videoId) {
        throw new ApiError("To delete a video from playlist  you should provide both playlist and video ids")
    }
...
const deletePlaylist = wrapAsyncHandler(async (req, res) => {
    ...
    return res
        .status(204)
        .json(204, {}, "Playlist has been deleted");
})
...
const updatePlaylist = wrapAsyncHandler(async (req, res) => {
    ...
    const playlist = await Playlist.findByIdAndUpdate(playlistId, updatedObject, { new: true });
    return res
    .status(200)
